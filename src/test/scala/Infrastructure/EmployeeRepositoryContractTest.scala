...
abstract class EmployeeRepositoryMEContractTest[F[_]](implicit ME:MonadError[F, Throwable])
  extends FunSpec with Matchers with BeforeAndAfterEach {
  ...
  def createRepositoryWith(employees: List[Employee]): EmployeeRepositoryME[F]
